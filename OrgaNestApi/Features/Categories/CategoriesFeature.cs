...
public class CategoriesController : ControllerBase
{
    ...
    {
        if (string.IsNullOrEmpty(request.Name)) return BadRequest("Category name is required.");
        ...
        if (existingCategory != null) return Conflict($"Category '{request.Name}' already exists.");
...
public class CategoryService : ICategoryService
{
    ...
    public async Task<Category> CreateCategoryAsync(CreateCategoryRequest dto, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrEmpty(dto.Name)) throw new ArgumentException("Category name is required.");

        // Check if category already exists
        var existingCategory = await GetCategoryByNameAsync(dto.Name, cancellationToken);
        if (existingCategory != null) throw new InvalidOperationException($"Category '{dto.Name}' already exists.");
